...
وقتی `t('home:title')` فراخوانی می‌کنیم، از namespace `home` کلید `title` را درخواست می‌دهیم. دونقطه namespace را از کلید جدا می‌کند. برای ترجمه‌های تو در تو مثل `t('home:meta.title')`، از dot notation برای ورود به آبجکت `meta` استفاده می‌کیم.
