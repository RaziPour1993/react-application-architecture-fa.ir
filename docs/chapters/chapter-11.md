# فصل ۱۱: تست اپلیکیشن {#h1_279 .chapterTitle}
...
- **End-to-end testها** کل اپلیکیشن را از منظر کاربر verify می‌کنند. سیستم کامل با فرانت‌اند و بک‌اند اجرا می‌شوند و تعاملات واقعی کاربر را شبیه‌سازی می‌کنند. این testها کندتر و هزینهٔ نگهداریشان بیشتر است، پس به‌صورت گزینشی استفاده می‌کنیم تا مسیرهای critical کاربر از ابتدا تا انتها درست کار کنند.
...
به‌خاطر هزینهٔ بالا، end-to-end test را به‌صورت گزینشی استفاده می‌کنیم. روی journeyهای critical کاربر تمرکز می‌کنیم — flowهای اصلی که برای مفید بودن اپلیکیشن‌مان باید کار کنند. برای اپلیکیشن ما، journeyهای critical عبارت‌اند از احراز هویت کاربر، ایجاد ایده، review کردن ایدهٔ شخص دیگر، و مدیریت پروفایل کاربر.
...
در نهایت، end-to-end test را پوشش دادیم که تأیید می‌کنند کل سیستم از ابتدا تا انتها کار می‌کند. این testها نگهداریشان گران است، پس گزینشی برای journeyهای critical کاربر استفاده می‌کنیم. وقتی pass می‌کنند، اطمینان می‌دهند اپلیکیشن‌مان در production کار می‌کند.
